            ...
            for s in sub_ann:
                for o in obj_ann:
                    if query_graphobj_mapping[item['queryid']].has_edge(s, o):
                        query_graphobj_mapping[item['queryid']][s][o]['weight'] = query_graphobj_mapping[item['queryid']][s][o]['weight'] + float(item['contextscore'])
                    else:
                        query_graphobj_mapping[item['queryid']].add_edge(s, o, weight=float(item['contextrank']))
        print(counter)
        counter = counter + 1
...
def relation_score_wrapper(input, lim, output):
    ...
    sorted_queryjson = sort_utils.sort_elements_by_value(queryjson, lim)
    output_list = conversion_utils.convert_entity_counter_dict_to_trec_format(sorted_queryjson, 'relation_edge_weight_score')
