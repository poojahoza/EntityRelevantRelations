...
public class ReadJSONFeatureGenerator {

    public ReadJSONFeatureGenerator(){
    ...
    private double[] entityCoMentions(String first_entity,
                                  String second_entity,
                                  Map<String, Map<Integer, List<String>>> bm25_ranking){
        double rel_score = 0.0;
        double counter = 0.0;
        for(Map.Entry<String, Map<Integer, List<String>>> c:bm25_ranking.entrySet()){
            for(Map.Entry<Integer, List<String>> c1:c.getValue().entrySet()) {
                List<String> para_entities = c1.getValue();
                //if (Arrays.stream(para_entities).anyMatch(first_entity::equals) && Arrays.stream(para_entities).anyMatch(second_entity::equals)) {
                boolean e1 = para_entities.contains(first_entity);
                boolean e2 = para_entities.contains(second_entity);
                if (e1 && e2) {
                    rel_score += (double) 1 / c1.getKey();
    ...
    private Map<String, Double[]> generateFeatureVectors(Map<String, Integer> entities_list,
                                                         String query_id,
                                                         Map<String, Map<String, Map<Integer, List<String>>>> bm25_ranking,
