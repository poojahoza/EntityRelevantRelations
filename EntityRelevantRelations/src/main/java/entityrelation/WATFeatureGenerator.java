...
import main.java.entitylinker.wat.WATEntityLinker;
...
public class WATFeatureGenerator {

    public WATFeatureGenerator(){
    ...
    private double[] entityCoMentions(String first_entity,
                                      ...
                                      Map<String, Map<String, EntityAnnotation>> bm25_ranking_wat_annotations){
        double rel_score = 0.0;
        double counter = 0.0;

        for(Map.Entry<String, Map<String, EntityAnnotation>> paras:bm25_ranking_wat_annotations.entrySet()){
            Set<String> wat_entities_ids = new HashSet<String>();
            for(Map.Entry<String, EntityAnnotation> paras_entities: paras.getValue().entrySet()){
                wat_entities_ids.add("enwiki:"+paras_entities.getValue().getTitle().replace("_","%20"));
            }
        ...
        /*for(Map.Entry<String, Container> c:bm25_ranking.entrySet()){
            ...
            if (Arrays.stream(para_entities).anyMatch(first_entity::equals) && Arrays.stream(para_entities).anyMatch(second_entity::equals)) {
                ...
                /*if(first_entity.equals("enwiki:Antibiotic%20use%20in%20livestock") || first_entity.equals("enwiki:Centers%20for%20Disease%20Control%20and%20Prevention") ||
                first_entity.equals("enwiki:Common%20cold") || first_entity.equals("enwiki:Food%20and%20Drug%20Administration") || first_entity.equals("enwiki:National%20Academy%20of%20Sciences") ||
                first_entity.equals("enwiki:National%20Institutes%20of%20Health") || first_entity.equals("enwiki:Natural%20Resources%20Defense%20Council") || first_entity.equals("enwiki:Preventive%20healthcare") ||
                first_entity.equals("enwiki:Respiratory%20tract%20infection") || first_entity.equals("enwiki:Self-medication")){

                    if(second_entity.equals("enwiki:Antibiotic%20use%20in%20livestock") || second_entity.equals("enwiki:Centers%20for%20Disease%20Control%20and%20Prevention") ||
                            second_entity.equals("enwiki:Common%20cold") || second_entity.equals("enwiki:Food%20and%20Drug%20Administration") || second_entity.equals("enwiki:National%20Academy%20of%20Sciences") ||
                            second_entity.equals("enwiki:National%20Institutes%20of%20Health") || second_entity.equals("enwiki:Natural%20Resources%20Defense%20Council") || second_entity.equals("enwiki:Preventive%20healthcare") ||
                            second_entity.equals("enwiki:Respiratory%20tract%20infection") || second_entity.equals("enwiki:Self-medication")){
                ...
                /*if(first_entity.equals("enwiki:Poaching")){
                    System.out.println("====="+first_entity+" "+second_entity+" "+c.getValue().getRank()+" "+String.valueOf((double)1 / c.getValue().getRank())+" "+counter);
                    if(second_entity.equals("enwiki:Poaching")){
                        System.out.println("====="+first_entity+" "+second_entity+" "+c.getValue().getRank()+" "+String.valueOf((double)1 / c.getValue().getRank())+" "+counter);
                    }
                }
    ...
    private Map<String, Double[]> generateFeatureVectors(Map<String, Integer> entities_list,
        ...
        Map<String, Map<String, EntityAnnotation>> wat_entity_annotations = new LinkedHashMap<>();
        for(Map.Entry<String, Container> c1:c2.entrySet()){
            Document rankedDoc = null;
            try{
                rankedDoc = indexSearcher.doc(c1.getValue().getDocID());
                WATEntityLinker watEntityLinker = new WATEntityLinker(rankedDoc.getField("Text").stringValue());
                watEntityLinker.annotateText();
                Map<String, EntityAnnotation> temp_wat_annotate = watEntityLinker.getEntitiesDetails();
                wat_entity_annotations.put(c1.getKey(), temp_wat_annotate);
            }catch (IOException ioe){
                ioe.printStackTrace();
            }
        }
        ...
        /*for(Map.Entry<String, Container> c1:c2.entrySet()){
            System.out.println(c1.getValue().getRank()+" "+c1.getValue().getEntity().getEntityVal());
            System.out.println("********************************************");
            Document rankedDoc = null;
            try {
                rankedDoc = indexSearcher.doc(c1.getValue().getDocID());
            }catch (IOException ioe){
                System.out.println("in indexsearcher io exception");
            }
            System.out.println(c1.getValue().getRank()+" "+rankedDoc.getField("Text").stringValue());
            System.out.println("++++++++++++++++++++++++++++++++++++++++++++");
        }*/
    ...
    public Map<String, Map<String, Double[]>> getFeatureVectors(Map<String, Map<String, Integer>> query_entity_list,
        ...
        /*query_entity_list.entrySet().parallelStream().forEach(m ->
        {
            if(m.getKey().equals("enwiki:Wildlife%20management/Management%20of%20hunting%20seasons/Closed%20season")){
                query_entity_feature_vec.put(m.getKey(), generateFeatureVectors(m.getValue(), m.getKey(), bm25_ranking, entity_ranking, indexLocation));
            } });*/
    ...
    public Map<String, Map<String, Double[]>> getNormalizedFeatureVectors(Map<String, Map<String, Integer>> query_entity_list,
        ...
        query_entity_normalized_vec = getFeatureVectors(query_entity_list, bm25_ranking, entity_ranking, indexLocation);
